...
//@Service
public class CustomRuleUnitProvider implements RuleUnitProvider {

    private static final Logger LOGGER = LoggerFactory.getLogger(CustomRuleUnitProvider.class);
...
//    @Autowired
    ...
    @Override
    public <T extends RuleUnitData> RuleUnitInstance<T> createRuleUnitInstance(T ruleUnitData) {
        this.packageName = ruleUnitData.getClass().getPackage().getName();
        return RuleUnitProvider.super.createRuleUnitInstance(ruleUnitData);
    }

    public CustomRuleUnitProvider() {
    ...
    private String drlContent = "package %s;\n" +
            "\n" +
            "\n" +
            "unit PersonRuleUnit;\n" +
            "\n" +
            "rule \"will execute per each Person having name\"\n" +
            "when\n" +
    ...
    InternalKieModule createRuleUnitKieModule(Class<?> unitClass, boolean useExecModel) {
        try {
            drlContent = String.format(drlContent,packageName);
            kieContainerFactory.addOrUpdateRule("will execute per each Person having name",drlContent,"person");
        } catch (IOException e) {
            throw new RuntimeException(e);
        }

        var kieMod = (InternalKieModule) kieContainerFactory.getLatestModule();

        return kieMod;
    }

    KieModuleKieProject createRuleUnitKieProject(InternalKieModule kieModule, boolean useExecModel) {
        return useExecModel ?
    ...
    private Collection<Resource> ruleResourcesForUnitClass(KieServices ks, Class<?> unitClass) {
    ...
    private void collectResourcesInFileSystem(KieServices ks, Class<?> unitClass, String unitStatement, Collection<Resource> resources, URL resourceUrl) {
    ...
    private boolean doesDrlContainUnit(File file, String unitStatement) {
        return file.getName().endsWith(".drl") && readFileAsString(file).contains(unitStatement);
    }

    private boolean doesXlsContainUnit(File file, String unitName) {
    ...
    private boolean doDecisionTablePropertiesContainUnit(Map<String, List<String[]>> dtableProperties, String unitName) {
    ...
    private void collectResourcesInJar(KieServices ks, Collection<Resource> resources, Class<?> unitClass, String unitStatement, URL resourceUrl) {
    ...
    private boolean doesDrlContainUnit(JarFile jarFile, JarEntry jarEntry, String unitStatement) {
        return jarEntry.getName().endsWith(".drl") && readJarEntryAsString(jarFile, jarEntry).contains(unitStatement);
    }

    private boolean doesXlsContainUnit(JarFile jarFile, JarEntry jarEntry, String unitName) {
    ...
    private <T extends RuleUnitData> boolean hasSameRuleUnitDataClass(RuleUnit ruleUnit, Class<T> ruleUnitDataClass) {
